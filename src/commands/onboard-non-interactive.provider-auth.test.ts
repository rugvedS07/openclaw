...
const ensureWorkspaceAndSessionsMock = vi.hoisted(() =>
  vi.fn(async (..._args: unknown[]) => {}),
);
const readConfigFileSnapshotMock = vi.hoisted(() =>
  vi.fn(async () => {
    const [{ default: fs }, { default: path }, { default: crypto }] =
      await Promise.all([
    ...
    if (!configPath) {
      throw new Error(
        "OPENCLAW_CONFIG_PATH must be set for provider auth onboarding tests",
      );
    }
...
const replaceConfigFileMock = vi.hoisted(() =>
  vi.fn(async (params: { nextConfig: unknown }) => {
    ...
    await fs.writeFile(
      configPath,
      `${JSON.stringify(params.nextConfig, null, 2)}\n`,
      "utf-8",
    );
    return {
      path: configPath,
...
const testAuthProfileStores = vi.hoisted(
  () =>
    new Map<
      string,
      { version: number; profiles: Record<string, Record<string, unknown>> }
    >(),
);
  ...
  const credential =
    ...
    typeof params.credential.key === "string"
      ...
      : params.credential.type === "token" &&
          typeof params.credential.token === "string"
        ? {
            ...params.credential,
...
vi.mock(
  "./onboard-non-interactive/local/auth-choice.plugin-providers.js",
  async () => {
    const [
      ...
      return {
        ...
        runNonInteractive: async (ctx) => {
          ...
          return params.defaultModel
            ? applyPrimaryModel(next, params.defaultModel)
            : next;
        },
      };
    }
    ...
    function extractLmstudioModelIds(payload: unknown): string[] {
      ...
      return models
        ...
        .filter((entry): entry is { type: string; key: string } =>
          Boolean(entry),
        )
    ...
    function createLmstudioChoice(): ChoiceHandler {
      return {
        ...
        runNonInteractive: async (ctx) => {
          const baseUrl =
            normalizeText(ctx.opts.customBaseUrl) || "http://localhost:1234/v1";
          ...
          const discoveredModelIds = extractLmstudioModelIds(
            await response.json(),
          );
          ...
          if (!discoveredModelIds.includes(selectedModelId)) {
            ctx.runtime.error(
              `LM Studio model ${selectedModelId} was not found at ${baseUrl}.`,
            );
            ctx.runtime.exit(1);
            return null;
          }
      ...
      [
        "setup-token",
        {
          ...
          async runNonInteractive(ctx) {
            ...
            upsertAuthProfile({
              profileId:
                (ctx.opts.tokenProfileId as string | undefined) ??
                "anthropic:default",
              credential: {
                type: "token",
                provider: "anthropic",
    ...
    return {
      applyNonInteractivePluginProviderChoice: async (params: {
        ...
        const workspaceDir =
          resolveAgentWorkspaceDir(enableResult.config, agentId) ??
          resolveDefaultAgentWorkspaceDir();
...
vi.mock("./onboard-helpers.js", async () => {
  const actual = await vi.importActual<typeof import("./onboard-helpers.js")>(
    "./onboard-helpers.js",
  );
  return {
    ...actual,
...
async function readCustomLocalProviderApiKey(
  configPath: string,
...
async function loadProviderAuthOnboardModules(): Promise<void> {
  ...
  ({ clearPluginManifestRegistryCache } = await import(
...
describe("onboard (non-interactive): provider auth", () => {
    ...
    await withZaiProbeFetch(
      ...
      async (fetchMock) =>
        await withOnboardEnv(
          "openclaw-onboard-zai-coding-global-",
          async (env) => {
            const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("openclaw-onboard-ai-gateway-", async (env) => {
      ...
      expect(cfg.auth?.profiles?.["vercel-ai-gateway:default"]?.mode).toBe(
        "api_key",
      );
      expect(cfg.agents?.defaults?.model?.primary).toBe(
  ...
  it("stores legacy Anthropic setup-token onboarding again when explicitly selected", async () => {
    await withOnboardEnv(
      ...
      async ({ configPath, runtime }) => {
        ...
        expect(
          getOrCreateTestAuthStore().profiles["anthropic:default"],
        ).toMatchObject({
          provider: "anthropic",
          type: "token",
          token: cleanToken,
  ...
  it("stores the detected env alias as keyRef for both OpenCode runtime providers", async () => {
    await withOnboardEnv(
      "openclaw-onboard-ref-opencode-alias-",
      async ({ runtime }) => {
        await withEnvAsync(
          {
  ...
  it.each([
    ...
    await withOnboardEnv(prefix, async ({ configPath, runtime }) => {
      ...
      expect(
        cfg.auth?.profiles?.["cloudflare-ai-gateway:default"]?.provider,
      ).toBe("cloudflare-ai-gateway");
      expect(cfg.auth?.profiles?.["cloudflare-ai-gateway:default"]?.mode).toBe(
        "api_key",
      );
      expect(cfg.agents?.defaults?.model?.primary).toBe(
        "cloudflare-ai-gateway/claude-sonnet-4-5",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("openclaw-onboard-together-infer-", async (env) => {
      ...
      expect(cfg.auth?.profiles?.["together:default"]?.provider).toBe(
        "together",
      );
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv(
      ...
      async ({ configPath, runtime }) => {
        ...
        expect(
          provider?.models?.some((model) => model.id === "foo-large"),
        ).toBe(true);
        expect(cfg.agents?.defaults?.model?.primary).toBe(
          "custom-llm-example-com/foo-large",
        );
      },
    );
  });
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      ...
      async ({ configPath, runtime }) => {
        ...
        expect(
          cfg.models?.providers?.["custom-models-custom-local"]?.baseUrl,
        ).toBe("https://models.custom.local/v1");
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      ...
      async ({ configPath, runtime }) => {
        ...
        expect(await readCustomLocalProviderApiKey(configPath)).toBe(
          "custom-env-key",
        );
      },
    );
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv(
      "openclaw-onboard-custom-provider-ref-flag-",
      async ({ runtime }) => {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv(
      "openclaw-onboard-custom-provider-invalid-id-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveSetupWithDefaults(runtime, {
            authChoice: "custom-api-key",
